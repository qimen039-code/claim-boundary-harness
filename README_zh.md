...
CBH 是给编码 AI 配套的一套工作规则、记忆整理方法和检查工具。它不替 AI 工作，
而是帮助 AI 在复杂和长期任务中少忘事、不串项目、不提前宣布完成、不反复犯同一种
错误，也不轻易把猜测写成已经确认的事实。
...
当前 main 分支版本：`v1.2.2`。
最新已打 tag 的 GitHub Release：[`v1.2.2`](https://github.com/qimen039-code/claim-boundary-harness/releases/tag/v1.2.2)。
