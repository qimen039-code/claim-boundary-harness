...
Archived capsules should keep source-monitoring fields when they exist: `source_tag` `belief_status` `confidence` `derived_from` `source_monitoring` `lifecycle` `belief_trace_summary`. Keep the index row compact and open payloads only after meta-level selection.

Archive retrieval results should still include `source_tag` `derived_from` `belief_status` `confidence` `score_method` so cold records do not re-enter active context as unbounded facts.
